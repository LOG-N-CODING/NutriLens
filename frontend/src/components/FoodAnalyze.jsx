    ...
    if (stopCommand.includes(input.toLowerCase())) {
      ...
      try {
        if (mode === 'chat') {
          ...
          addMessage('bot', `📸 Image analysis completed! Found: ${images.length} food item(s).\n\n🔧 Backend TODO: Implement image analysis endpoint`, false, true);
          setImages([]);
        } else if (mode === 'hybrid') {
          // Hybrid Mode - Text + Image
          const analysisText = input.trim() ? `Text: "${input}"` : '';
          const imageText = images.length > 0 ? `Images: ${images.length} uploaded` : '';
          const combinedText = [analysisText, imageText].filter(Boolean).join(' + ');
          addMessage('bot', `🔬 Hybrid analysis completed!\n\n📝 ${combinedText}\n\n🔧 Backend TODO: Implement hybrid analysis endpoint`, false, true);
          setImages([]);
        }
      } catch (err) {
